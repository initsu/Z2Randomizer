...
#pragma warning restore CS8618 
        ...
        if (direction == Direction.EAST || direction == Direction.WEST)
        {
            if (y < MAP_ROWS / 2)
            {
                for (int i = 0; i < olength; i++)
                {
                    ...
                    {
                        logger.Debug("DrawOcean could not add water west");
                        return false;
                    }
                    map[y + i, x] = water;
                }
            ...
            else
            {
                try
                {
                    for (int i = 0; i < olength; i++)
                    {
                        ...
                        if (t != Terrain.NONE && biome != Biome.MOUNTAINOUS && biome != Biome.CALDERA)
                        {
                            logger.Debug("DrawOcean could not add water east");
                            return false;
                        }
                        map[y - i, x] = water;
                    }
        ...
        else // north or south
        {
            if (x < MAP_COLS / 2)
            {
                for (int i = 0; i < olength; i++)
                {
                    ...
                    if (t != Terrain.NONE && biome != Biome.MOUNTAINOUS && biome != Biome.CALDERA)
                    {
                        logger.Debug("DrawOcean could not add water north");
                        return false;
                    }
                    map[y, x + i] = water;
                }
            ...
            else
            {
                for (int i = 0; i < olength; i++)
                {
                    ...
                    if (t != Terrain.NONE && biome != Biome.MOUNTAINOUS && biome != Biome.CALDERA)
                    {
                        logger.Debug("DrawOcean could not add water south");
                        return false;
                    }
                    map[y, x - i] = water;
                }
    ...
    protected bool FindPassthroughCave(IntVector2 pos, IntVector2 dir, Location cave1, Location cave2)
    {
        ...
        while (true)
        {
            ...
            {
                logger.Debug("Start cave reached end of map");
                return false;
            }
        ...
        while (true)
        {
            ...
            if (!WithinMapBounds(pos, 1))
            {
                logger.Debug("Ending cave reached end of map");
                return false;
            }
            if (map[pos.Y, pos.X] != Terrain.MOUNTAIN)
            {
        ...
        if (length <= 2)
        {
            logger.Debug("Cave passthrough path not long enough");
            return false;
        }

        if (HasAdjacentCave(pos))
        {
            logger.Debug("Cave exit is too close to existing cave");
            return false;
        }
    ...
    private IntVector2? MoveAwayIfNextToCave(IntVector2 pos, IntVector2 sideDir)
    {
        ...
        while (true)
        {
            ...
            if (++i == 6)
            {
                logger.Debug("Start cave too close to another cave");
                return null;
            }
        }
    }
    ...
    public IntVector2? ValidBlockingCavePosition(IntVector2 pos)
    {
        ...
        bool checkForExistingLocation(IntVector2 pos)
        {
            ...
            {
                var loc = GetLocationByCoords((pos.Y, pos.X));
                return loc != null;
            }
            return false;
        }
    ...
    public IntVector2? ValidTrapTilePosition(IntVector2 pos)
    {
        ...
        {
            var p = pos + i * dir;
            var l = GetLocationByCoords((p.Y, p.X));
            if (l != null)
            {
                return null;
            }

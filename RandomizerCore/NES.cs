...
public class NES
{
    ...
    public static (byte dark, byte middle, byte light) RollMatchingColorTriple(Random r, List<int>? darkRange)
    {
        ...
        List<int> darkList = darkRange != null ? darkRange : [.. Enumerable.Range(0x00, 0x0d), .. Enumerable.Range(0x11, 0x0d)];
        dark = darkList.Sample(r);
        ...
        while (lightList.Count > 0)
        {
            ...
            float hueDiffDark = dark == 0x00 || dark == 0x1d ? 0f : hueDistance(lightHue, darkHue);

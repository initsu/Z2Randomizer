    ...
    */
    ...
    private void RerollPaletteTable(int paletteTableAddr, Random r)
    {
        ...
        if (paletteTableAddr == RomMap.TOWN_PALETTE_TABLE)
        {
            ...
            (dark, middle, light) = NES.RollMatchingColorTriple(r, null);
        ...
        foreach (List<int> list in tripples)
        {
            ...
            List<int>? darkRange = list == orangeSpriteColorAddr ? [.. Enumerable.Range(0x11, 7), .. Enumerable.Range(0x19, 4)] : null;
            if (paletteTableAddr == RomMap.GP_PALETTE_TABLE_MAJOR) { darkRange?.Remove(0x14); darkRange?.Remove(0x15); } // too close life color for dark link fight
            do
            {
                (dark, middle, light) = NES.RollMatchingColorTriple(r, darkRange);
            } while (usedColors.Contains(dark) || usedColors.Contains(middle));

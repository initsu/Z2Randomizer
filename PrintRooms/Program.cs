...
public class Program
{
    ...
    public static bool PrintRoom<T,U>(Room room, SideviewEditable<T> sv, EnemiesEditable<U> ee) where T : Enum where U : Enum
    {
        ...
        {
            DrawEnemy(canvas, bgPaint, e, room);
        }
    ...
    private static void DrawEnemy<T>(SKCanvas canvas, SKPaint bgPaint, Enemy<T> enemy, Room room) where T : Enum
    {
        int startX = enemy.X;
        int startY = enemy.Y;
        SKBitmap? tile = null;
        ...
        if (enemy.IsShufflableLarge())
        {
            ...
            {
                var sv = new SideviewEditable<GreatPalaceObject>(room.SideView);
                var solidGrid = sv.CreateSolidGrid();
                if (PalaceEnemyShuffler.PositionDoomknocker(ref enemy, solidGrid))
                {
                    tile = LoadChr(palaceTiles, 0x12e00, 2, 4, paletteBlue!);
                }
                else
                {
        ...
        if (enemy.IsShufflableFlying())
        {
            if (enemy is Enemy<EnemiesGreatPalace>)
            {
                var sv = new SideviewEditable<GreatPalaceObject>(room.SideView);
                var solidGrid = sv.CreateSolidGrid();
                if (PalaceEnemyShuffler.PositionKingBot(ref enemy, room, solidGrid))
                {
                    startY = enemy.Y;
                    /*
                    tile = LoadChr(palaceTiles, 0x8b40, 1, 2, paletteBlue!);
                    SKRect rect = new SKRect(startX * 16, startY * 16, startX * 16 + 24, startY * 16 + 32);
                    canvas.DrawBitmap(tile, rect);
                    tile = FlipTileHorizontally(tile);
                    SKRect rect2 = new SKRect(startX * 16 + 24, startY * 16, startX * 16 + 48, startY * 16 + 32);
                    canvas.DrawBitmap(tile, rect2);
                    */
                    tile = LoadChr(palaceTiles, 0xcf00, 3, 4, paletteBlue!);
                    canvas.DrawBitmap(tile, startX * 16, startY * 16);
                    tile = FlipTileHorizontally(tile);
                    canvas.DrawBitmap(tile, startX * 16 + 24, startY * 16);
                    return;
                }
            }
            if (enemy is Enemy<EnemiesGreatPalace> || enemy.IdByte != EnemiesRegularPalaceShared.ORANGE_MOA)
            {
